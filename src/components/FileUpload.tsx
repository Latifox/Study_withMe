...
const FileUpload = ({ courseId, onClose }: FileUploadProps) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      supabase.functions.invoke('extract-pdf-text', {
      ...
      }).then(async ({ error: extractionError, data: extractionData }) => {
        if (extractionError) {
          console.error('PDF extraction error:', extractionError);
          toast({
            title: "Warning",
            description: "Content generation encountered an error. Please try again later.",
            variant: "destructive",
          });
          return;
        }

        // Continue with segment structure generation
        ...
        if (segmentError) {
          ...
          toast({
            title: "Warning",
            description: "Error generating lecture segments. Please try again later.",
            variant: "destructive",
          });
          return;
        }

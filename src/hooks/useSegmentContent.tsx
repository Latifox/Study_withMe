...
export const useSegmentContent = (numericLectureId: number | null) => {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const processSegment = async (segment: { sequence_number: number; title: string; segment_description: string }) => {
      ...
      for (const segment of segmentData) {
        try {
          ...
          // Immediately return with whatever content we have so far
          if (results.length > 0) {
            return { segments: results };
          }
          throw error; // If we have no results at all, throw the error
        }
      }
